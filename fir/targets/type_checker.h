...
namespace fir
{
  ...
  public:
    type_checker(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<fir::symbol> &symtab, basic_ast_visitor *parent) : basic_ast_visitor(compiler), _symtab(symtab), _parent(parent)
    {
    }
  ...
  protected:
    ...
    void do_logical_operation(cdk::binary_operation_node *const node, int lvl);
    template <typename T>
